import threading
from PI1.simulators.dus1 import run_dus1_simulator
import time
...
def run_door_ultrasonic_simulator(settings, threads, stop_event ):
    if settings['simulated']:
        ...
        from PI1.sensors.dus1 import run_ultrasonic_sensor_loop, DUS1
        print("Starting DUS1 loop")
        dus1 = DUS1(settings['pin_trig'], settings['pin_echo'])
        dus1_thread = threading.Thread(target=run_ultrasonic_sensor_loop, args=(5, dus1, dus1_callback, stop_event, publish_event, settings))
        dus1_thread.start()
        threads.append(dus1_thread)
